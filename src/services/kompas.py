...
class KompasScrapService(ScrapInterface):
    ...
    def get_article(self):
        ...
        for i in range(self.page_number):
            ...
            try:
                driver = find_dynamic_articles(self.keyword, page, ScrapperMedia.kompas)
    ...
    def write_document_to_files(self):
        ...
        Path(path_folder + 'kompas/').mkdir(parents=True, exist_ok=True)

        # get existing filenames
        existing_files = os.listdir(path_folder + 'kompas/')
        ...
        for article in self.articles:
            ...
            write_kompas_article(article, path_folder, article_filename)
...
def set_prefix_filename(page, soup, article_filename):
    prefix = ''
    date_tag = soup.select('meta[property="article:published_time"]')
    if len(date_tag) > 0:
...
def create_metadata(soup, article_filename, link, article):
    ...
    try:
        metadata.author = soup.select('meta[name="content_author"]')[0].attrs['content']
    except KeyError:
        print("Article '" + article_filename + "' doesn't have author")

    # write title
    metadata.title = article.title
    metadata.media = 'kompas'
...
def write_kompas_article(article, path_folder, article_filename, page=1, prefix=''):
    # prepare link
    link = article.link
    ...
    txt_file, yml_file = create_path_result(path_folder, prefix, article_filename, ScrapperMedia.kompas)
    ...
    if pages > 1 and page < pages:
        write_kompas_article(article, path_folder, article_filename, page + 1, prefix)


def kompas_scrape(keyword, page_number, folder):
    print('scrap {} on kompas'.format(keyword))
    scrap_service = KompasScrapService(keyword, int(page_number), folder)
